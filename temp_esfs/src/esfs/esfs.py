...
def Create_Scaled_Matrix(adata,clip_percentile=97.5,log_scale=False):
    ...
    if issparse(adata.X) == False:
        print("Replacing adata.X counts matrix with a sparse csc array")
        adata.X = csc_matrix(adata.X)
    # Filter genes with no expression
    if issparse(adata.X) == True:
        Keep_Genes = adata.var_names[np.where(adata.X.getnnz(axis=0) > 50)[0]]
...
def Get_Overlap_Info(Fixed_Feature,Fixed_Feature_Cardinality,Sample_Cardinality,Feature_Sums,FF_QF_Vs_RF):
    ...
    if (Sub_Global_Scaled_Matrix.indices.shape[0] > 0) and (Non_Zero_Inds.shape[0] > 0):
        ...
        Overlaps = np.zeros(Global_Scaled_Matrix.shape[1])
    ...
    if (Sub_Global_Scaled_Matrix.indices.shape[0] > 0) and (Non_Zero_Inds.shape[0] > 0):
        ...
        Inverse_Overlaps = np.zeros(Global_Scaled_Matrix.shape[1])
    ####
...
def ES_CCF(adata,Secondary_Features_Label):
    """
...
def ES_FMG(adata,N,Secondary_Features_Label,Input_Genes=np.array([]),Num_Reheats=3,Resolution=1,Use_Cores=-1):
    """
...
def knn_Smooth_Gene_Expression(
    adata, Use_Genes, knn=30, metric='correlation', log_scale=False, batch_size=1000, n_jobs=-1
):
    assert metric == 'correlation', "Currently only 'correlation' metric is supported."

    # Get indices of selected genes
    Use_Genes_Inds = np.where(np.isin(np.asarray(adata.var_names), Use_Genes))[0]

    # Subset expression data to selected genes
    X_subset = adata[:, Use_Genes_Inds].X
    ...
    print(f"Computing batched correlation distances for {n_cells} cells, batch size = {batch_size}")
    ...
    batch_ranges = [(i, min(i + batch_size, n_cells)) for i in range(0, n_cells, batch_size)]
    ...
    # Compute smoothed expression by averaging neighbors
    Smoothed_Expression = np.array([
        np.mean(Full_X[neighbor_idx], axis=0)
        for neighbor_idx in neighbors
    ])

    adata.layers["Smoothed_Expression"] = csc_matrix(Smoothed_Expression.astype("f"))

    print(f"Smoothed expression with knn = {knn} saved to adata.layers['Smoothed_Expression']")
    return adata

# def knn_Smooth_Gene_Expression(adata, Use_Genes, knn=30, metric='correlation', log_scale=False): ### THIS HAD A BUG. Wasn't log transforming for distance calc
#     #
#     Use_Genes_Inds = np.where(np.isin(np.asarray(adata.var_names),Use_Genes))[0]
...
#         if log_scale == False:
#             distmat = squareform(pdist(Smoothed_Expression[:,Use_Genes_Inds], metric))
#         if log_scale == True:
#              distmat = squareform(pdist(np.log2(Smoothed_Expression[:,Use_Genes_Inds]+1), metric))
#     #     
#     else:
...
def ES_Rank_Genes(adata,EP_Threshold=0,ESS_Threshold=0.01,Exclude_Genes=np.array([]),Known_Important_Genes=np.array([]),Secondary_Features_Label="Self",Min_Edges=5):
    ##
...
def Plot_Top_Ranked_Genes_UMAP(adata,Top_Ranked_Genes,Clustering="None",Known_Important_Genes=np.array([]),UMAP_min_dist=0.1,UMAP_Neighbours=20,hdbscan_min_cluster_size=50,Secondary_Features_Label="Self",random_state=42):
    ...
    with warnings.catch_warnings():
        ...
        Gene_Embedding = umap.UMAP(n_neighbors=UMAP_Neighbours, min_dist=UMAP_min_dist, n_components=2, random_state=random_state).fit_transform(Masked_ESSs)
    ##
    # No clustering
    if Clustering == "None":
...
def Get_Gene_Cluster_Cell_UMAPs(adata,Gene_Clust_Labels,Top_ESS_Genes,n_neighbors,min_dist,log_transformed,specific_cluster=None,metric="correlation",random_state=None):
    ...
    for i in np.arange(Unique_Gene_Clust_Labels.shape[0]):
        ...
        if random_state != None:
            Embedding_Model = umap.UMAP(n_neighbors=n_neighbors, metric=metric,min_dist=min_dist,n_components=2,random_state=random_state).fit(Reduced_Input_Data)
        else:
            Embedding_Model = umap.UMAP(n_neighbors=n_neighbors, metric=metric,min_dist=min_dist,n_components=2).fit(Reduced_Input_Data)
...
def Plot_Gene_Cluster_Cell_UMAPs(
    ...
    if Cell_Label in adata.obs.columns:
        ...
        for idx, Embedding in enumerate(Gene_Cluster_Embeddings):
            ...
            ax = axes[row][col]
            ax.set_title(f"Gene cluster {idx} ({Gene_Cluster_Selected_Genes[idx].shape[0]} genes)", fontsize=14)
            for label in Unique_Cell_Labels:
                IDs = np.where(Cell_Labels == label)
                ax.scatter(Embedding[IDs, 0], Embedding[IDs, 1], s=marker_size, label=label)
            ax.set_xticks([])
            ax.set_yticks([])
        ...
        outer_grid = gridspec.GridSpec(nrow, ncol + 1, width_ratios=[1]*ncol + [0.05], wspace=0.3)
        ...
        for idx, Embedding in enumerate(Gene_Cluster_Embeddings):
            ...
            sc = ax.scatter(Embedding[Order, 0], Embedding[Order, 1], s=marker_size, c=Expression[Order], cmap="seismic")
            ax.set_title(f"Gene cluster {idx} ({Gene_Cluster_Selected_Genes[idx].shape[0]} genes)", fontsize=12)
            ax.set_xticks([])
            ax.set_yticks([])
            ax.set_xlabel("UMAP 1", fontsize=10)
            ax.set_ylabel("UMAP 2", fontsize=10)
