...
# More easily map sparse matrix types to their corresponding cupy sparse matrix types
SPARSE_CONVERSIONS = {
    "csc": (spsparse.csc_matrix, xpsparse.csc_matrix),
    "csr": (spsparse.csr_matrix, xpsparse.csr_matrix),
    "coo": (spsparse.coo_matrix, xpsparse.coo_matrix),
    "dia": (spsparse.dia_matrix, xpsparse.dia_matrix),
}
...
def create_scaled_matrix(adata, clip_percentile=97.5, log_scale=False):
    ...
    # NOTE: Sometimes, numpy is fine, this isn't performance-critical code
    ...
    upper[xp.where(upper == 0)[0]] = xp.max(scaled_expressions, axis=0)[xp.where(upper == 0)[0]]
...
def parallel_calc_es_matrices(
    ...
    # NOTE: .A is needed here, but incompatible with cupy.
    if not USING_GPU:
...
def calc_es_metrics(feature_ind, sample_cardinality, feature_sums, minority_states):
    ...
    FF_QF_vs_RF[xp.where(fixed_feature_minority_state > minority_states)[0]] = (
        1  # 1's mean FF is QF
    )
...
def ESE1(x, SD, RFm, RFM, QFm, QFM, Ts, max_overlap):
    ...
    """
    G1_E = (RFm / Ts) * (
        (((x) / RFm) * (-xp.log((x) / RFm))) + (((RFm - x) / RFm) * (-xp.log((RFm - x) / RFm)))
    )
    G2_E = (RFM / Ts) * (
        (((QFm - x) / RFM) * (-xp.log((QFm - x) / RFM)))
        + (((RFM - QFm + x) / RFM) * (-xp.log((RFM - QFm + x) / RFM)))
    )
    CE = xp.where(xp.isnan(G1_E), 0, G1_E) + xp.where(xp.isnan(G2_E), 0, G2_E)
    ind_E = (QFm / Ts) * (-xp.log((QFm / Ts))) + (QFM / Ts) * (-xp.log((QFM / Ts)))
    #
    min_E = xp.zeros(SD.shape[0])
    SD_1_idxs = xp.where(SD == -1)[0]
    min_E[SD_1_idxs] = (RFM[SD_1_idxs] / Ts) * (
        (((QFm[SD_1_idxs]) / RFM[SD_1_idxs]) * (-xp.log((QFm[SD_1_idxs]) / RFM[SD_1_idxs])))
        + (
            ((RFM[SD_1_idxs] - QFm[SD_1_idxs]) / RFM[SD_1_idxs])
            * (-xp.log((RFM[SD_1_idxs] - QFm[SD_1_idxs]) / RFM[SD_1_idxs]))
        )
    )
    SD1_idxs = xp.where(SD == 1)[0]
    min_E[SD1_idxs] = (RFM[SD1_idxs] / Ts) * (
        (
            ((QFm[SD1_idxs] - max_overlap[SD1_idxs]) / RFM[SD1_idxs])
            * (-xp.log((QFm[SD1_idxs] - max_overlap[SD1_idxs]) / RFM[SD1_idxs]))
        )
        + (
            ((RFM[SD1_idxs] - QFm[SD1_idxs] + max_overlap[SD1_idxs]) / RFM[SD1_idxs])
            * (-xp.log((RFM[SD1_idxs] - QFm[SD1_idxs] + max_overlap[SD1_idxs]) / RFM[SD1_idxs]))
        )
    )
    min_E[xp.isnan(min_E)] = 0
    min_E[xp.isnan(min_E)] = 0
    #
    CE[xp.isnan(CE)] = min_E[xp.isnan(CE)]
...
def ESE2(x, SD, RFm, RFM, QFm, QFM, Ts):
    ...
    """
    G1_E = (RFm / Ts) * (
        (-(((RFm - x) / RFm) * xp.log((RFm - x) / RFm)) - (((x) / RFm) * xp.log((x) / RFm)))
    )
    G2_E = (RFM / Ts) * (
        (
            -(((RFM - QFM + x) / RFM) * xp.log((RFM - QFM + x) / RFM))
    ...
    min_E[SD1_idxs] = (RFM[SD1_idxs] / Ts) * (
        (
            -(
                ...
                * xp.log((RFM[SD1_idxs] - QFM[SD1_idxs] + RFm[SD1_idxs]) / RFM[SD1_idxs])
            )
            - (
                ((QFM[SD1_idxs] - RFm[SD1_idxs]) / RFM[SD1_idxs])
                * xp.log((QFM[SD1_idxs] - RFm[SD1_idxs]) / RFM[SD1_idxs])
            )
        )
    )
    min_E[xp.isnan(min_E)] = 0
    ...
    min_E[SD_1_idxs] = (RFM[SD_1_idxs] / Ts) * (
        (
            -(
                ...
                * xp.log((min_overlap[SD_1_idxs]) / RFM[SD_1_idxs])
            )
            - (
                ((RFM[SD_1_idxs] - min_overlap[SD_1_idxs]) / RFM[SD_1_idxs])
                * xp.log((RFM[SD_1_idxs] - min_overlap[SD_1_idxs]) / RFM[SD_1_idxs])
            )
        )
    )
    SD1_idxs = xp.where(SD == 1)[0]
    ...
    min_E[SD1_idxs] = (RFM[SD1_idxs] / Ts) * (
        (
            -(
                ...
                * xp.log((RFM[SD1_idxs] - max_overlap[SD1_idxs]) / RFM[SD1_idxs])
            )
            - (
                ((max_overlap[SD1_idxs]) / RFM[SD1_idxs])
                * xp.log((max_overlap[SD1_idxs]) / RFM[SD1_idxs])
            )
        )
    )
    min_E[xp.isnan(min_E)] = 0
